        ...
        <div class="wrap">
            ...
            <div class="pub-shell">
                ...
                <div class="col" style="gap: 16px;">
                    ...
                    <div class="card flat" x-data="{ linkCopied: false }">
                            ...
                            <div>
                                ...
                                <div class="row" style="margin-top: 10px; flex-wrap: wrap; gap: 8px;">
                                    <button type="button" class="btn sm"
                                            @click="navigator.clipboard.writeText('{{ url('/piggy/' . $user->piggy_code) }}').then(() => { linkCopied = true; setTimeout(() => linkCopied = false, 2000) })">
                                        ...
                                        <span x-show="linkCopied" x-cloak>✓ Copied!</span>
                                    </button>
                                    <a href="https://wa.me/?text={{ urlencode('Send a gift to ' . $user->name . '\'s Piggy Wallet: ' . url('/piggy/' . $user->piggy_code)) }}"
                                    ...
                                    <a href="{{ route('piggy.download-qr.public', $user->piggy_code) }}" class="btn sm">
                                    ...
                                    <button type="button" class="btn sm"
                                            @click="
                                                const url = '{{ url('/piggy/' . $user->piggy_code) }}';
                                                if (navigator.share) {
                                                    navigator.share({ title: '{{ addslashes($user->name) }}\'s Piggy Wallet', text: 'Send a gift or payment to {{ addslashes($user->name) }} via MyPiggyBox', url }).catch(() => {});
                                                } else {
                                                    navigator.clipboard.writeText(url).then(() => { linkCopied = true; setTimeout(() => linkCopied = false, 2000) });
                                                }
                                            ">
