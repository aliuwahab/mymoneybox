...
<style>
  ...
  .box-card { background: #FAFAF7; border: 1px solid #E6E3DC; border-radius: 10px; padding: 14px 18px; margin: 0 0 4px; }
  .box-name { font-size: 15px; font-weight: 700; color: #15140F; }
  .body { padding: 28px 32px 32px; }
  .share-section-label { font-size: 13px; font-weight: 600; color: #15140F; margin-bottom: 12px; }
  .share-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 20px; }
  .share-btn { display: inline-block; text-decoration: none; padding: 11px 20px; border-radius: 7px; font-size: 13px; font-weight: 600; color: #FFFFFF; }
  .share-btn-wa { background: #25D366; }
  .share-btn-fb { background: #1877F2; }
  .share-btn-tw { background: #15140F; }
  .qr-section { background: #FAFAF7; border: 1px solid #E6E3DC; border-radius: 10px; padding: 16px 20px; margin-bottom: 20px; display: flex; gap: 14px; align-items: center; flex-wrap: wrap; }
  .qr-text { font-size: 13px; color: #6B6862; flex: 1; min-width: 160px; }
...
@php
    ...
    $waText = urlencode('Check out my PiggyBox "' . $moneyBox->title . '" and contribute: ' . $boxUrl);
    $fbUrl  = urlencode($boxUrl);
    $twText = urlencode('Support my PiggyBox "' . $moneyBox->title . '" 🐷');
    $twUrl  = urlencode($boxUrl);
    $firstName = explode(' ', $moneyBox->user->name)[0];
@endphp
<div class="wrap">
  <div class="header">
  ...
  <div class="hero">
    <div class="hero-tag">Your PiggyBox is live</div>
    <h2>Time to get the word out</h2>
    <p>Hi {{ $firstName }}, your PiggyBox is live and ready to receive contributions. The next step is sharing it with your network.</p>
    <div class="box-card">
  ...
  <div class="body">
    <p class="share-section-label">Share via</p>
    ...
    <div class="qr-section">
      ...
      <div class="qr-links">
        <a href="{{ route('money-boxes.download-qr', $moneyBox) }}" class="cta">Download QR</a>
        <a href="{{ route('money-boxes.share', $moneyBox) }}" class="cta-outline">Share page</a>
      </div>
    </div>
    ...
    <div class="tips-section">
      ...
      <div class="tip">
        ...
        <div class="tip-text"><strong style="color:#15140F;">Post to your Instagram story</strong> — Add your PiggyBox link to your story with a swipe-up or link sticker so followers can contribute on the spot.</div>
      </div>
      <div class="tip">
    ...
    <div class="notice">
      ...
      @if($moneyBox->user->piggy_code)
        <br><br>
        <a href="{{ route('piggy.show', ['code' => $moneyBox->user->piggy_code]) }}">{{ route('piggy.show', ['code' => $moneyBox->user->piggy_code]) }}</a>
      @endif
    </div>
  </div>

  <div class="footer">
    <p>You're receiving this because you created a PiggyBox on <a href="{{ config('app.url') }}">{{ config('app.name') }}</a>. <a href="{{ route('money-boxes.share', $moneyBox) }}">View share &amp; QR page</a></p>
  </div>
</div>
</body>
</html>